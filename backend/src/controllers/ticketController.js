...
export const ticketController = {
  ...
  async getAllTickets(req, res) {
    try {
      const tickets = await ticketService.getAllTickets(req.auth?.user?.id || null);
  ...
  async getTicketById(req, res) {
    try {
      const ticket = await ticketService.getTicketById(req.params.id, req.auth?.user?.id || null);
  ...
  async getMeusChamadosByAuthUser(req, res) {
    try {
      ...
      const authUserEmail = (req.auth?.user?.email || '').trim() || null;
      const page = Number(req.query?.page) || 1;
      const pageSize = Number(req.query?.pageSize) || 50;
      const result = await ticketService.getMeusChamadosByAuthUser(authUserId, authUserEmail, { page, pageSize });
  ...
  async getReceivedTickets(req, res) {
    try {
      ...
      res.status(500).json({ success: false, error: 'Erro ao listar chamados recebidos', message: error?.message || String(error) });
    }
  },
  ...
  async getReceivedConcludedTickets(req, res) {
    try {
      ...
      res.status(500).json({ success: false, error: 'Erro ao listar chamados concluídos', message: error?.message || String(error) });
    }
  },
