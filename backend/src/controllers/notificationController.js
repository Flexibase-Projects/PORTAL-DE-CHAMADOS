...
export const notificationController = {
  async list(req, res) {
    try {
      ...
      console.error('[notificationController] list:', error?.message || error);
      res.status(500).json({ success: false, error: 'Erro ao carregar notificações', message: error?.message || String(error) });
    }
  },
